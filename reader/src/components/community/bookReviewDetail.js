...
class BookReviewDetail extends Component {
  ...
  componentDidMount() {
    ...
    InteractionManager.runAfterInteractions(() => {
      ...
      dispatch(
        bookReviewDetailCommentList(_id, {start: 0, limit: 30}, true, []),
      );
    });
  }
  ...
  _showMoreItem() {
    ...
    if (
      bookReview.bookReviewCommentList.length === 0 ||
    ...
    dispatch(
      bookReviewDetailCommentList(
        this.state.id,
        {start: bookReview.bookReviewCommentList.length, limit: 30},
        false,
        bookReview.bookReviewCommentList,
      ),
    );
  }
  ...
  renderBookReviewComment({item: rowData, index}) {
    return (
      <View style={styles.item}>
        <Image
          style={styles.itemImage}
          ...
          <View>
            <View
              ...
              <Image
                ...
                source={
                  bookReview.bookReviewDetail.author.avatar
                    ? {
                        uri:
                          api.IMG_BASE_URL +
                          bookReview.bookReviewDetail.author.avatar,
              ...
              <View style={styles.authorBody}>
                <Text style={styles.authorTitle}>
                  {bookReview.bookReviewDetail.author.nickname}
                </Text>
                <Text style={styles.authorCreateTime}>
                  {dateFormat(bookReview.bookReviewDetail.created)}
                </Text>
              </View>
            </View>
            <Text style={styles.detailTitle}>
              {bookReview.bookReviewDetail.title}
            </Text>
            <Text style={styles.detailContent}>
              {bookReview.bookReviewDetail.content}
            </Text>
            <TouchableOpacity
              activeOpacity={0.5}
              onPress={() =>
                this._goToBookDetail(bookReview.bookReviewDetail.book._id)
              }>
              <View style={styles.bookBody}>
                <Image
                  style={{height: 60, width: 45, alignSelf: 'center'}}
                  source={
                    bookReview.bookReviewDetail.book.cover
                      ? {
                          uri:
                            api.IMG_BASE_URL +
                            bookReview.bookReviewDetail.book.cover,
                ...
                <View style={{marginLeft: 14, alignSelf: 'center'}}>
                  <Text
                    ...
                    {bookReview.bookReviewDetail.book.title}
                  </Text>
                  <View style={{flexDirection: 'row', marginTop: 10}}>
                    <Text
                      style={{
    ...
    return (
      <View style={styles.container}>
        ...
        {bookReview.isLoadingBookReviewCommentList ? (
          ...
          <FlatList
            enableEmptySections={true}
            data={bookReview.bookReviewCommentList}
            onEndReached={this._showMoreItem.bind(this)}
            onEndReachedThreshold={30}
            renderItem={this.renderBookReviewComment.bind(this)}
