...
export default class BookListTab extends Component {
  ...
  _showMoreItem() {
    if (
      this.state.bookLists.length === 0 ||
      this.state.isLoading ||
      this.state.isLoadingMore ||
      this.state.bookLists.length === this.state.total
    ) {
      return;
    }
    let params = this._setTabParams(
      ...
      this.props.tag,
      this.props.gender,
      this.state.bookLists.length,
    );
    this._getBookListTabDetail(params);
  }

  _goToBookListDetail(rowData) {
    this.props.navigation.push('BookListDetail', {
